...
const FIELD_CLASSNAME =
  "booking-field min-h-12 w-full rounded-2xl border border-foreground/10 bg-background px-4 py-3 text-base text-foreground outline-none transition-colors placeholder:text-foreground/40 focus:border-[var(--color-primary)] focus:bg-foreground/[0.03]";
...
export function BookingForm({
  ...
  const formCard = (
    <div className="booking-card overflow-hidden rounded-[28px] border border-foreground/8 bg-foreground/[0.04] ring-1 ring-foreground/5">
      <div className="booking-card-inner relative px-5 py-6 sm:px-6 sm:py-7">
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[var(--color-primary)]/70 to-transparent" />

        {isSuccess ? (
          <div className="booking-success flex min-h-[20rem] flex-col items-start justify-center">
            ...
            <h3 className="booking-success-title mt-5 text-2xl font-black leading-tight tracking-[-0.04em] text-foreground">
              Спасибо! Мы свяжемся с вами в ближайшее время
            </h3>
            <p className="booking-success-copy mt-3 max-w-xl text-sm leading-6 text-foreground/70 sm:text-base">
              Заявка уже передана администратору клиники. Если вопрос срочный, вы также можете написать в WhatsApp.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit(onSubmit)} className="booking-form-fields space-y-4 sm:space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div ref={serviceMenuRef} className="relative">
                ...
                {isServiceMenuOpen ? (
                  <div
                    role="listbox"
                    className="booking-select-panel absolute z-20 mt-2 max-h-64 w-full overflow-y-auto rounded-2xl border border-foreground/10 bg-background p-2 text-foreground shadow-[0_24px_60px_color-mix(in_oklab,var(--color-primary)_12%,transparent)] ring-1 ring-foreground/5"
                  >
            ...
            {submitError ? (
              <div className="booking-feedback rounded-2xl border border-foreground/10 bg-foreground/[0.03] px-4 py-3 text-sm leading-6 text-foreground">
                {submitError}
              </div>
            ) : null}
            ...
            <button
              ...
              className="booking-submit inline-flex min-h-12 w-full items-center justify-center rounded-2xl bg-[var(--color-primary)] px-5 py-3 text-base font-semibold text-white shadow-[0_14px_36px_color-mix(in_oklab,var(--color-primary)_26%,transparent)] transition-colors disabled:cursor-not-allowed disabled:opacity-70"
            >
  ...
  if (isSheet) {
    return (
      <div className="booking-sheet space-y-4">
        <div>
          <p className="booking-kicker text-xs font-medium uppercase tracking-[0.28em] text-[var(--color-primary)]/82">
            {title}
          </p>
          <h3 className="booking-title mt-3 text-2xl font-black leading-[1.02] tracking-[-0.04em] text-foreground">
            Оставьте заявку без звонка
          </h3>
          <p className="booking-subtitle mt-3 text-sm leading-6 text-foreground/72">{subtitle}</p>
        </div>
        {formCard}
      </div>
    );
  ...
  return (
    <section id="booking-form" className="booking-form-section bg-background py-16 sm:py-20 lg:py-24">
      <div className="booking-shell mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(24rem,0.92fr)] lg:items-start">
          <div className="max-w-xl">
            <p className="booking-kicker text-xs font-medium uppercase tracking-[0.28em] text-[var(--color-primary)]/82">
              {title}
            </p>
            <h2 className="booking-title mt-3 text-3xl font-black leading-[0.98] tracking-[-0.04em] text-foreground sm:text-4xl lg:text-5xl">
              Оставьте заявку, и мы свяжемся с вами
            </h2>
            <p className="booking-subtitle mt-4 text-base leading-7 text-foreground/72 sm:text-lg">
              {subtitle}
            </p>
          </div>
